...
# Answer TRUE or FALSE in the variable below
...
result_q3 <- average_dance_energy(df$danceability[1], df$energy[1])
...
q4df <- df %>%
  ...
  filter(energy >= 0.5) %>%
  group_by(artist) %>%
  summarize(across(c(popularity, danceability, energy), mean, na.rm = TRUE)) %>%
