...
# Access the information of the 30th through the 45th cars and the 47th, and 79th cars in the dataset.
...
origin_year_columns <- df[ , "origin year"]
...
columns_1_to_4_acceleration <- df[ , c(names(df)[1:4], "acceleration")]


# Combined Access ----
# Access the mpg and year for the first five cars in the dataset.
mpg_year_first_five <- df [1:5, c("mpg", "year")]
  
# Access the horsepower and weight of cars 10 to 15 in df?
horsepower_weight_10_to_15 <- df[10:15, c("horsepower", "weight")]
  
# Access the mpg, horsepower, and year for the 20th, 25th, and 30th cars in the dataset.
mpg_horsepower_year_20_25_30 <- df[c(20,25,30), c("mpg", "horsepower", "year")]

# Extract the cylinders, displacement, and acceleration for cars
# with indices 30 through the 45 and the 47th, and 79th cars.
cylinders_displacement_acceleration_30_to_45_47_79 <- df[
  c(30:45,47,79), c("cylinders", "displacement", "acceleration")
] 

# Access the first 4 columns and acceleration for cars at positions 
#2 to 4, and 6 in the dataset?

columns_1_to_4_acceleration_2_to_4_6 <- df[c(2:4, 6), c(names(df)[1:4], "acceleration")]
