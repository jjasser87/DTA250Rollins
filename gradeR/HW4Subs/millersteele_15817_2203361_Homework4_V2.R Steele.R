...
area_triangle <- area_triangle <- function(base, height) {
  0.5 * base * height
}
...
celsius_to_fahrenheit <- celsius_to_fahrenheit <- function(celsius) {
  fahrenheit <- (celsius * 9/5) + 32
  if (fahrenheit > 90) {
    return(paste(fahrenheit, "It's hot!"))
  } else {
    return(paste(fahrenheit, "It's not hot!"))
  }
}
