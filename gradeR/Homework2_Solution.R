...
origin_year_columns <- df[,c("origin", "year")]
...
columns_1_to_4_acceleration <- df[,c(1:4, 6)] # We use 6 instead of acceleration because you can't mix column numbers and column names in the same vector
...
cylinders_displacement_acceleration_30_to_45_47_79 <- df[c(30:45, 47, 79), c("cylinders", "displacement", "acceleration")]

# Access the first 4 columns and acceleration for cars at positions 2 to 4, and 6 in the dataset
columns_1_to_4_acceleration_2_to_4_6 <- df[c(2, 3, 4, 6), c(1:4, 6)] # We use 6 instead of acceleration because you can't mix column numbers and column names in the same vector
```
