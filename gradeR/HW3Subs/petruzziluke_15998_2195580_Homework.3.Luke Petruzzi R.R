...
plot1 <- # You code for the plot here
...
plot2 <- # Your code for the plot here
  plot2 <- ggplot(diamonds, aes(x = carat, y = price, color = cut)) +
  geom_point(alpha = 0.5) + # Make points semi-transparent
  geom_smooth(method = "lm", se = FALSE, aes(color = cut)) + # Add linear model lines for each 'cut'
...
plot3 <- # Your code for the plot here
  plot3 <- ggplot(mtcars, aes(x = factor(cyl), y = mpg)) +
  geom_boxplot() +
...
# Set the y parameter to "Count".
   ...
   plot4 <- ggplot(iris, aes(x = Petal.Length)) +
    geom_histogram(bins = 30) + # Set number of bins to 30
    facet_wrap(~ Species) + # Create separate histograms for each species
    labs(title = "Petal Length Distribution for Each Species",
         x = "Petal Length",
         y = "Count")

plot4 <- # Your code for the plot here
