...
plot1 <- # plot1 <- ggplot(penguins, aes(x = bill_length_mm)) +
  geom_density() +
  facet_grid(~ species) +
...
plot2 <- # plot2 <- ggplot(diamonds, aes(x = carat, y = price)) +
...
plot3 <- # plot3 <- ggplot(mtcars, aes(x = factor(cyl), y = mpg)) +
  geom_boxplot() +
...
plot4 <- # plot4 <- ggplot(iris, aes(x = Petal.Length)) +
