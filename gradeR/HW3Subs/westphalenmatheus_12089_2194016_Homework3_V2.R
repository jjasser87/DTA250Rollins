...
plot1 <- ggplot(penguins, mapping=aes(x=bill_length_mm)) + 
  geom_density() +
  facet_grid(~ species) +
...
plot2 <- ggplot(diamonds, mapping=aes(x = carat, y = price)) +
...
plot3 <- ggplot(mtcars, mapping=aes(x=factor(cyl), y=mpg)) +
  geom_boxplot() +
...
plot4 <- ggplot(iris, mapping=aes(x=Petal.Length)) +
