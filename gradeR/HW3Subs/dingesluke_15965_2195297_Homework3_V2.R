...
plot1 <- ggplot(data = penguins, mapping = aes(x = bill_length_mm)) +
  geom_density() + facet_grid(~ species) + 
  labs(title = "Density Plot of Bill Length by Species", x = "Bill Length (mm)",
       y = "Density")
...
plot2 <- ggplot(data = diamonds, mapping = aes(x = carat, y = price)) +
  geom_point(mapping = aes(color = cut), alpha = 0.5) + geom_smooth(method = lm) +
  labs(title = "Carat vs. Price by Cut Quality", x = "Carat", y = "Price")
...
plot3 <- ggplot(data = mtcars, mapping = aes(x= factor(cyl), y =mpg)) +
  geom_boxplot() + labs(title = "Miles Per Gallon by Cylinder Number",
...
plot4 <- ggplot(data = iris, mapping = aes(x = Petal.Length)) +
  geom_histogram(bins = 30) + facet_wrap(~ Species) + labs(title = "Petal Length Distribution for Each Species",
