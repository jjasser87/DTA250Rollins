...
plot2 <- ggplot(data = diamonds, mapping = aes(x = carat, y = price)) +
  ...
  geom_smooth(mapping = aes(color = cut), method = "lm") +
...
plot3 <- ggplot(data = mtcars, mapping = aes(x = factor(cyl), y = mpg)) +
  geom_boxplot() +
  labs(title = "Miles Per Gallon by Cylinder Number", x = "Number of Cylinders", y = "Miles Per Gallon (mpg)")
...
plot4 <- ggplot(data = iris, mapping = aes(x = Petal.Length)) +
  ...
  facet_wrap(~ Species) +
  labs(title = "Petal Length Distribution for Each Species", x = "Petal Length", y = "Count")
