...
# No we want to create a histogram plot of every variable in the Boston dataset.
...
Boston_long <- pivot_longer(Boston, cols = everything(), names_to = "Variable", values_to = "Value")
...
all_plot <- ggplot(Boston_long, mapping = aes(x = Value)) + 
  geom_histogram() + 
