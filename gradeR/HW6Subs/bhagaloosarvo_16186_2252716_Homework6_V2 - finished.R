...
# No we want to create a histogram plot of eeery variable in the Boston dataset.
...
# Set the names_to parameter to "Variable" to name the new column "Variable"
# Set the values_to parameter to "Value" to name the new column "Value"

pivot_longer(data = Boston, cols = everything(), names_to = "Variable", values_to = "Value")

Boston_long <-  pivot_longer(data = Boston, cols = everything(), names_to = "Variable", values_to = "Value")
...
all_plot <- ggplot(data = Boston_long, aes(x = Value)) +
                   geom_histogram() +
