...
plot <- ggplot(df, mapping=aes(x = danceability, y = energy)) + 
        geom_point() +
...
q4df <- select(df, artist, popularity, danceability, energy) |>
  filter(energy >= 0.5) |>
  group_by(artist) |>
  summarize(mean_pop = mean(popularity), mean_dance = mean(danceability), mean_energy = mean(energy)) |>
...
most_popular_artist <- df |>
  group_by(artist) |>
  summarize(mean_pop = mean(popularity)) |>
  slice_max(order_by = mean_pop, n=1) |>
  select(artist)
