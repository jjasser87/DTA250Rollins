...
average_dance_energy <- function(danceability, energy) {
  if (average_dance_energy = mean((danceability, energy), na.rm = T) > 0.5) {
    return("High")
  } else if (average_dance_energy = mean((danceability, energy), na.rm = T) <= 0.5) {
    return("Low")
  }
}


average_dance_energy(1)
??lessthan
average_dance_energy(df$danceability[1], df$energy[1])
average_dance_energy(df$danceability[0.1], df$energy[0.2])
# Question 4 ----
# 10 points
...
q4df <- df %>%
  ...
  filter() %>%
  group_by(artist) %>%
  summarise(avg_pop_dance_energy = mean(popularity, na.rm = T),
            mean(danceability, na.rm = T), mean(energy, na.rm = T)) %>%
  arrange("popularity")
