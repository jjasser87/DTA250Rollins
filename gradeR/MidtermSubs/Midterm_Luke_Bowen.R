...
rows_5_to_10_20_to_22 <- df[c(5:10,20:22),c("energy","key","loudness")]
...
class_popularity_5th_is_numeric <- FALSE
 is.numeric(class_popularity_5th_is_numeric)
# Question 2 ----
# 10 points
...
plot <- ggplot(data=df, aes(x=danceability,y=energy)) +
   geom_point()+
   geom_smooth (method = "lm")+
   facet_wrap(~ key)+
   labs(title="Scatter Plot of Danceability and Energy",x="Danceability",y="Energy")
...
average_dance_energy <- df(danceability,energy) |>
  group_by(danceability, energy) |>
...
most_popular_artist <- "XXXTENTACION"
