...
class_popularity_5th_is_numeric <- function(class_popularity_5th){
  if(class_popularity_5th == "numeric"){
  return("True")
}
else{
  return("False")
...
plot <- ggplot(data = df, mapping = aes(x = danceability, y = energy)) +
  ...
  facet_wrap(~ key) +
  labs(title = "Scatter Plot of Daceability and Energy", x = "Danceability", y = "Energy")
...
average_dance_energy <- function(dance, energy){
  dance_energy_avg <- mean(c(dance, energy))
  if (dance_energy_avg > 0.5){
    return("High")
  } 
  else {
    return("Low")
...
q4df <- df |> select(artist, popularity, danceability, energy) |>
  filter(energy >= 0.5) |>
  group_by(artist) |>
  summarize(avg = mean(c(popularity, danceability, energy))) |>
