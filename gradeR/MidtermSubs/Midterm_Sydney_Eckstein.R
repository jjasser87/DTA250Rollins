...
class_popularity_5th_is_numeric <- TRUE #integers are also numeric
...
average_dance_energy(df$danceability[193], df$energy[193])
...
q4df <- df |> 
  ...
  filter(energy >= 0.5) |>
  group_by(artist) |>
  summarize(mean_popularity = mean(popularity), mean_danceability = mean(danceability), mean_energy = mean(energy)) |>
  arrange(desc(mean_popularity)) #mean calculated in previous column/ popularity?
