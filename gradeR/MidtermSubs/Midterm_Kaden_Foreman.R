...
plot <- ggplot(data=df, mapping=aes(x=danceability, y=energy))+geom_point()+geom_smooth(method=lm)+
  facet_wrap(~key)+labs(title="Scatter Plot of Danceability and Energy",
                        x="Danceability",
                        y="Energy")
...
average_dance_energy <- function(x,y) {
  avg<-(x+y)/2
  if(avg>.5) {
    return("High")
  }else {
...
q4df <- df|>
  ...
  filter(energy>=.5) |>
  group_by(artist)|>
  summarise(avg_popularity=mean(popularity), avg_danceability=mean(danceability),avg_energy=mean(energy))|>
