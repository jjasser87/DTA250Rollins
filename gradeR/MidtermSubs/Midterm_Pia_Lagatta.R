...
popularity_5th <- df[5, c("popularity")]
...
average_dance_energy <- function(x) {
  if (x > 0.5) {
    return("High")
  } else if (x <= 0.5){
    return("Low")
...
average_dance_energy(average_dance_energy(df$danceability[1], df$energy[1]))
...
q4df <- df %>%
  select(df, "artist", "popularity", "danceability", "energy") %>%
  filter(artist = "artist", popularity = "popularity", danceability = "danceability", energy = "energy") %>%
  slice_max(order_by(slice_max(n = any_of(n >= 0.5)))),
  group_by(artist) %>%
    summarise(avg_popularity = mean(popularity_5th, na.rm = T), 
              avg_danceability = mean(danceability, na.rm = T),
              avg_energy = mean(energy, na.rm = T),
  arrange(pick(starts_with("popularity", desc))))
