...
class_popularity_5th_is_numeric <- is.numeric(popularity_5th)
...
plot <- ggplot(df, mapping = aes(x = "danceaility", y = "energy")) +
  geom_point()+
...
average_dance_energy <- average_dance_energy()
mean(df$danceability, df$energy)
function(average_dance_energy) {
  if (average_dance_energy > 0.5) {
    return("High")
  } else {
    return("Low")
  }
...
# Use the group_by() and summarize() functions to group by the "artist" column 
# and calculate the average of the specified columns.
# Finally, arrange the dataset in descending order of the "popularity" column.

q4df <- df |>
  filter(energy > 0.5) |>
  group_by(artist) |>
  summarize(average_populatrity = mean(popularity), 
            average_danceability = mean(danceability), average_energy = mean(energy))
