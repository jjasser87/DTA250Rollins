...
rows_5_to_10_20_to_22 <- df[c(5:10, 20:22), c("danceability", "energy", "loudness")]
...
plot <- ggplot(data = df, aes(x = danceability, y = energy)) +
  ...
  facet_wrap("~key") +
  labs(title = "Scatter Plot of Danceability", x = "Danceability", y = "Energy")
...
q4df <- df |>
  ...
  filter(energy >= 0.5) |>
  group_by(artist) |>
  summarise(avgpop = mean(popularity), avgdance = mean(danceability), avgenergy = mean(energy)) |>
  arrange(desc(avgpop))
