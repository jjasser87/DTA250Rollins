...
class_popularity_5th_is_numeric <- FALSE #Because 65 has an L, which means its an integer.
...
plot <- 
  ggplot(data = df, mapping = aes(x = danceability, y = energy)) +
  geom_point() +
...
average_dance_energy <- function(df$danceability[1], df$energy[1]){
...
most_popular_artist <- c("Britney Spears")
