...
popularity_5th <- df$popularity[5]
...
plot <- ggplot(data = df, aes(x = danceability, y = energy)) + geom_point() + geom_smooth(method = "lm") + facet_wrap(~ key) + labs(title = "Scatter Plot of Danceability and Energy", x = "Danceability", y = "Energy")
...
average_dance_energy <- function(dance,energy), avg <- (dance) + (energy) / 2
