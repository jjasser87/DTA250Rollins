...
rows_5_to_10_20_to_22 <- dc[c(5:10, 20:22), c("danceability", "energy", "key", "loudness")]
...
class_popularity_5th_is_numeric <- class_popularity_5th == "numeric"
...
facet_wrap(~ key) +
labs(title = "Scatter Plot of Danceability and Energy", x= "danceability", y = "Energy") plot
...
average_dance_energy <- function(dance, energy) (avg > 0.5) {return("High")}
 else {return("Low")}
# Question 4 ----
# 10 points
...
q4df <- df %>%
select(artist, populatiry, danceability, energy) %>%
filter(energy >= 0.5) %>%
group_by(artist) %>%
summarize(avergae_popularity= mean(popularity), average_danceability = mean(danceability), avergae_energy = mean(energy)) %>%
arrange(desc(average_populatiry))
