...
average_dance_energy <- function(dance, energy) {
  avg_dance_and_energy <- mean(dance + energy)
  if (avg_dance_and_energy > 0.5) {
    return("High")
  } else {
    return("Low")
  }
...
average_dance_energy(df$danceability[2], df$energy[2])
average_dance_energy(df$danceability[26], df$energy[26])
