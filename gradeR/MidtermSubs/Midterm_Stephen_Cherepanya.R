...
rows_5_to_10_20_to_22 <- df[c(5, 6, 7, 8, 9, 10, 20, 21, 22), c(7, 8, 9, 10)]

# Multiply popularity_5th by rows_5_to_10_20_to_22
result <- popularity_5th * (c(5, 6, 7, 8, 9, 10, 20, 21, 22), )
...
average_dance_energy <- function(average) {
  if (average > 0.5) {
    return ("High") 
  } else if (average < 0.5) {
    return ("Low")
...
# Finally, arrange the dataset in descending order of the "popularity" column. COME BACK

q4df <- df |>
  select(artist, popularity, danceability, energy) |>
  filter(energy > 0.5) |>
  group_by("artist") |>
  summarize(mean(q4df, na.rm = T)) |>
