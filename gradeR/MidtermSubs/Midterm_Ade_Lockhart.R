...
class_popularity_5th_is_numeric <- as.logical(TRUE)
...
average_dance_energy <- function(df)
  summarize(mean(df[,7]))
  (mean(df[,8]))

  if (average_dance_energy(df$danceability[1], df$energy[1]) > 0.5) {
    sprintf("High")
  }else {
    sprintf("Low")
  }
...
q4df <- select(df[, c(1,6:8)]) 
  filter(df, energy >= 0.5)
  group_by(df, artist) %>%
  summarize(mean(popularity, na.rm = TRUE))
