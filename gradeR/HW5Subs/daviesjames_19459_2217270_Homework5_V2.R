...
f# Filter the cars that has more than 100 horsepower and has more than 4 cylinders
...
more_than_100_hp_less_than_20_mpg_more_than_6_cyl <- filter(mtcars, hp > 100 & mpg < 20 & cyl > 6) %>%
  arrange(desc(mpg))
...
avg_mpg_by_cyl_gear <- mtcars %>% 
  group_by(cyl, gear) %>% 
  summarize(mean(mpg, na.rm = TRUE), .groups = "drop")
