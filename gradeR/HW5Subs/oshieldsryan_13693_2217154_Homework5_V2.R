...
avg_percollege_percbelowpoverty
...
setosa_filter_avg_sepal_length
...
sep_width_avg_by_species <- iris |>
  group_by(Species) |>
  summarize(avg_sep_width = mean(Sepal.Width))

sep_width_avg_by_species
...
avg_sepal_length_petal_length_by_species                                  
...
more_than_100_hp_more_than_4_cyl
...
more_than_100_hp_less_than_20_mpg_more_than_6_cyl <- filter(mtcars, hp > 100 &
                                                              mpg < 20 &
                                                              cyl > 6) |>
  arrange(desc(mpg)) 
