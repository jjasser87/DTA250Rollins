...
il_filter_percollege <- midwest |>
  summarize(mean(il_filter$percollege))
...
all_states_percollege <- midwest |>
  group_by(state) |>
  summarize(mean(percollege, na.rm = T))
...
std_dev_percchildbelowpovert_percbelowpoverty <- midwest |>
  group_by(state) |>
  summarize(sd(percbelowpoverty, na.rm = T))
...
setosa_filter_avg_sepal_length <- iris |>
  summarize(mean(setosa_filter$Sepal.Length))
...
sep_width_avg_by_species <- iris |>
  group_by(Species) |>
  summarize(mean(Sepal.Width, na.rm = T))
...
avg_sepal_length_petal_length_by_species <- iris |>
  group_by(Species) |>
  summarize(mean(Sepal.Length, na.rm = T), mean(Petal.Length, na.rm = T))
