...
all_states_percollege <-  midwest |> group_by(state) |> summarize(percollege_mean = mean(percollege))
...
avg_percollege_percbelowpoverty <- midwest |>
  summarize(percollege_avg  = mean(percollege), belowpov_avg = mean(percbelowpoverty))


  
# calculate the median population (popadults) for each state. use the median() function
median_popadults <- midwest |> group_by(state) |> summarize(median_pop_by_state = median(popadults))
...
std_dev_percchildbelowpovert_percbelowpoverty <- midwest |> group_by(state) |> summarize(sd_percchildbelowpovert = sd(percchildbelowpovert), sd_percbelowpov = sd(percbelowpoverty) )
...
sep_width_avg_by_species <- iris |> group_by(Species) |> summarize(species_mean = mean(Sepal.Width))
...
avg_sepal_length_petal_length_by_species <- iris |> group_by(Species) |> summarize(Sepal_length_avg = mean(Sepal.Length), Petal_length_avg = mean(Petal.Length))
...
avg_mpg_by_cyl_gear <- mtcars |> group_by(cyl, gear) |> 
...
avg_mpg_by_cyl_gear_carb <- mtcars |> group_by(cyl, gear, carb) |> 
  summarize(avg_mpg = mean(mpg), .groups = "drop"  ) |>
  arrange(desc(cyl))
