...
il_filter <- midwest |> filter(state == "IL")# Your code here
...
il_filter_percollege <- il_filter |> summarize(avg = mean(percollege))# Your code here
...
all_states_percollege <- midwest |> 
  group_by(state) |> 
  summarise(all_avg = mean(percollege)) # Your code here
...
avg_percollege_percbelowpoverty <- midwest |>
  group_by(state) |> 
  summarise(state_avg = mean(percollege), pov_avg = mean(percbelowpoverty))# Your code here
  
# calculate the median population (popadults) for each state. use the median() function
median_popadults <- midwest |>
  group_by(state) |>
  summarise(pop_med = median(popadults))# Your code here
...
std_dev_percchildbelowpovert_percbelowpoverty <- midwest |> 
  group_by(state) |>
  summarise(sdchild = sd(percchildbelowpovert), sdcollege = sd(percbelowpoverty))# Your code here
...
setosa_filter <- iris |> filter(Species == "setosa") # Your code here
...
setosa_filter_avg_sepal_length <- setosa_filter |> 
  summarise(avglength = mean(Sepal.Length))# Your code here
