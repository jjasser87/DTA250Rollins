...
all_states_percollege <- average_percollege <- midwest |>
  group_by(state) |>
  summarize(all_states_percollege = mean(percollege, na.rm = TRUE))
...
sep_width_avg_by_species <- iris |>
  group_by(Species) |>
  summarize(avg_sepal_width = mean(Sepal.Width, na.rm = TRUE))
...
avg_sepal_length_petal_length_by_species <- iris |>
  group_by(Species) |>
  summarize(avg_sepal_length = mean(Sepal.Length, na.rm = TRUE),
