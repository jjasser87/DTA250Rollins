...
std_dev_percchildbelowpovert_percbelowpoverty <- midwest %>%
  group_by(state) %>%
  summarize(sd_percchildbelowpovert = sd(percchildbelowpovert), sd_percbelowpoverty = sd(percbelowpoverty))
...
avg_sepal_length_petal_length_by_species <- iris %>%
  group_by(Species) %>%
  summarize(avg_sepal_length = mean(Sepal.Length), avg_petal_length = mean(Petal.Length))
