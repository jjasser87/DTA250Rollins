...
all_states_percollege <- midwest |>
  group_by(state) |>
  summarize(ave_percollege = mean(percollege , na.rm = TRUE))
...
avg_percollege_percbelowpoverty <- midwest |>
  group_by(state) |>
  summarize(ave_percollege = mean(percollege , na.rm = TRUE),
            ave_percbelowpoverty = mean(percbelowpoverty, na.rm = TRUE))
...
setosa_filter_avg_sepal_length <- setosa_filter |>
  summarize(
    ave_Sepal.length = mean(Sepal.Length, na.rm =TRUE)
  )
...
sep_width_avg_by_species <- iris |>
  group_by(Species)|>
  summarize(
    ave_Sepal.Width = mean(Sepal.Width, na.rm =TRUE)
  )
...
avg_sepal_length_petal_length_by_species <- iris |>
  ...
  summarise(
    ave_Sepal.Length = mean(Sepal.Length, na.rm =TRUE),
    ave_Petal.Length = mean(Petal.Length, na.rm =TRUE)
  )
...
more_than_100_hp_less_than_20_mpg_more_than_6_cyl <- 
  filter(mtcars, hp > 100, mpg< 20, cyl > 6)|>
  arrange(desc(mpg))
