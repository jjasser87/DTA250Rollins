...
# observations where the state = is equal to "IL"
...
avg_percollege_percbelowpoverty <- midwest |>
  summarize(mean(percollege, na.rm=TRUE), mean(percbelowpoverty, na.rm=TRUE))
...
sep_width_avg_by_species <- iris|>
  group_by(Species) |>
  summarize(avg_Sepal.Width=mean(Sepal.Width))
