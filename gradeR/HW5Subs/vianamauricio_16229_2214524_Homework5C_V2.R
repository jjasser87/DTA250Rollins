...
il_filter_percollege <- il_filter |>
  summarise(avgpcollege = mean(percollege))# Your code here
...
all_states_percollege <- midwest |>
  group_by(state) |>
  summarise(avg_percollege = mean(percollege, na.rm = TRUE))# Your code here
...
avg_percollege_percbelowpoverty <- midwest |>
  group_by(state) |>
  summarise(avgpc = mean(percollege, na.rm = TRUE), 
            avgperbelow = mean(percbelowpoverty, na.rm = TRUE)) # Your code here
  
# calculate the median population (popadults) for each state. use the median() function
median_popadults <- midwest |>
  group_by(state) |>
  summarise(medianpop = median(popadults))# Your code here
...
std_dev_percchildbelowpovert_percbelowpoverty <- midwest |>
  group_by(state) |>
  summarise(sdperchildb = sd(percchildbelowpovert), sdpercbelow = sd(percbelowpoverty))# Your code here
...
setosa_filter_avg_sepal_length <- setosa_filter |>
  summarise(avgsepallen = mean(Sepal.Length))# Your code here
...
sep_width_avg_by_species <- iris |>
  group_by(Species) |> 
  summarise(avgwidth = mean(Sepal.Width))# Your code here
...
avg_sepal_length_petal_length_by_species <- iris |>
  group_by(Species) |>
  summarise(avgpetal = mean(Petal.Length), avgsepal = mean(Sepal.Length))# Your code here
...
avg_mpg_by_cyl <- mtcars |>
  group_by(cyl) |>
  summarise(avgmpg = mean(mpg))# Your code here
