...
il_filter_percollege <- midwest |>
  summarise(avg_percollege = mean(percollege, na.rm = T))
...
std_dev_percchildbelowpovert_percbelowpoverty <- midwest |>
  group_by(state) |>
  summarise(std_dev_percchildbelowpovert = sd(percchildbelowpovert),
            std_dev_percbelowpoverty = sd(percbelowpoverty))
...
avg_mpg_by_cyl_gear_carb <- mtcars |>
  ...
  summarise(avg_mpg_by_cyl_gear_carb = mean(mpg)) |>
  arrange(cyl)
