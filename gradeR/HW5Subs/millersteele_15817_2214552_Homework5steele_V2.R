...
il_filter <- midwest |> filter(state == "IL")
...
il_filter_percollege <- il_filter |> summarise(avg_percollege = mean(percollege))
...
all_states_percollege <- midwest |> summarise(avg_percollege = mean(percollege))
...
avg_percollege_percbelowpoverty <- midwest |> summarise(avg_percollege = mean(percollege), avg_percbelowpoverty = mean(percbelowpoverty))


  
# calculate the median population (popadults) for each state. use the median() function
median_popadults <- midwest |> summarise(median_popadults = median(popadults))
...
std_dev_percchildbelowpovert_percbelowpoverty <- midwest |> summarise(sd_percchildbelowpovert = sd(percchildbelowpovert), sd_percbelowpoverty = sd(percbelowpoverty))
...
setosa_filter <- iris |> filter(Species == "setosa")

# Can you find the average of the Sepal.Length column for the filtered dataset (setosa_filter)?
# Use the mean() function

setosa_filter_avg_sepal_length <- setosa_filter |> summarise(avg_sepal_length = mean(Sepal.Length))
...
sep_width_avg_by_species <- iris |> group_by(Species) |> summarise(avg_sepal_width = mean(Sepal.Width))
...
avg_sepal_length_petal_length_by_species <- iris |> group_by(Species) |> summarise(avg_sepal_length = mean(Sepal.Length), avg_petal_length = mean(Petal.Length))

# Mtcars dataset
?mtcars # to give you an idea of the columns in the mtcars dataset

# Filter the cars that has more than 100 horsepower
more_than_100_hp <- mtcars |> filter(hp > 100)

# Filter the cars that has more than 100 horsepower and has more than 4 cylinders
more_than_100_hp_more_than_4_cyl <- mtcars |> filter(hp > 100, cyl > 4)
...
more_than_100_hp_less_than_20_mpg_more_than_6_cyl <- mtcars |> filter(hp > 100, mpg < 20, cyl > 6) |> arrange(desc(mpg))
...
avg_mpg_by_cyl_gear <- mtcars |> group_by(cyl, gear) |> summarise(avg_mpg = mean(mpg))


# Calculate the average miles per gallon for each number of cylinders and each number of gears
# and each number of carburetors. Arrange them by the number of cylinders in ascending order
avg_mpg_by_cyl_gear_carb <- mtcars |> group_by(cyl, gear, carb) |> summarise(avg_mpg = mean(mpg)) |> arrange(cyl)
