...
avg_percollege_percbelowpoverty <- midwest|>
  group_by(state)|>
  summarize(mean_percollege=mean(percollege), mean_perbelowpoverty=mean(percbelowpoverty))
  
# calculate the median population (popadults) for each state. use the median() function
median_popadults <- midwest|>
  group_by(state)|>
  summarise(median_population=median(popadults))
...
std_dev_percchildbelowpovert_percbelowpoverty <- midwest|>
  group_by(state)|>
  summarise(sd_perchildbelowpovert=sd(percchildbelowpovert))
