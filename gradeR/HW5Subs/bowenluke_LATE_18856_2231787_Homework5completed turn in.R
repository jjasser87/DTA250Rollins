...
std_dev_percchildbelowpovert_percbelowpoverty <- midwest|>
  group_by(percchildbelowpovert, percbelowpoverty)|>
  summarize(std_dev_percchildbelowpovert=sd(percchildbelowpovert),std_dev_percbelowpoverty=sd(percbelowpoverty))
...
avg_sepal_length_petal_length_by_species <- iris |>
  group_by(Species) |>
  summarise(across(c(Sepal.Length, Petal.Length), mean, .names = "avg"))
