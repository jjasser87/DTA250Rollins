...
std_dev_percchildbelowpovert_percbelowpoverty <-
...
avg_sepal_length_petal_length_by_species <- iris %>%
  group_by(Species) %>%
  summarize(avg_petal_length = mean(Sepal.Length),
...
more_than_100_hp <- subset(mtcars, hp > 100)

# Filter the cars that has more than 100 horsepower and has more than 4 cylinders
more_than_100_hp_more_than_4_cyl <- subset(mtcars, hp > 100 & cyl > 4)
...
avg_mpg_by_cyl_gear <- aggregate(mpg ~ cyl + gear, data = mtcars, FUN = mean)
