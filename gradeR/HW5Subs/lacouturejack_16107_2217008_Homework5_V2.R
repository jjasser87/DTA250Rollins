...
avg_percollege_percbelowpoverty <- midwest |>
  group_by(state) |>
  summarize(avg_percollege = mean(percollege), avg_percbelowpoverty = mean(percbelowpoverty))
  
# calculate the median population (popadults) for each state. use the median() function
median_popadults <- midwest |>
  group_by(state) |> 
  summarize(med_popadults = median(popadults))
...
std_dev_percchildbelowpovert_percbelowpoverty <-  midwest |>
  group_by(state) |>
  summarize(stddev_percbelowpoverty = sd(percbelowpoverty))
...
sep_width_avg_by_species <- iris |>
  group_by(Species) |>
  summarize(avg_sepalwidth = mean(Sepal.Width))
...
avg_sepal_length_petal_length_by_species <- iris |>
  group_by(Species) |>
  summarize(avg_sepallength = mean(Sepal.Length), avg_petallength = mean(Petal.Length))
