...
il_filter_percollege <- midwest |>
  filter(state == "IL") |>
  summarise(mean_per_college_IL = mean(percollege))
...
all_states_percollege <- midwest |> 
  group_by(state) |>
  summarise(avg_percollege = mean(percollege))
...
avg_percollege_percbelowpoverty <- midwest |>
  group_by(state) |>
  summarise(avg_percollege = mean(percollege), avg_percbelowpoverty = mean(percbelowpoverty))
  
# calculate the median population (popadults) for each state. use the median() function
median_popadults <- midwest |>
  group_by(state) |>
  summarise(median_pop = median(popadults))
...
avg_mpg_by_cyl_gear <- mtcars |>
  group_by(cyl, gear) |>
  summarise(avg_mpg_cyl_gear = mean(mpg))
