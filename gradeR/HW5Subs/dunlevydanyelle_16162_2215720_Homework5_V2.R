...
avg_percollege_percbelowpoverty <- midwest |>
  group_by(state) |>
  summarize(
    average_percollege = mean(percollege, na.rm = TRUE),
    average_percbelowpoverty = mean(percbelowpoverty, na.rm = TRUE)
  )
...
median_popadults <- midwest |>
  group_by(state) |>
  summarize(
    median_popadults = median(popadults, na.rm = TRUE)
  )
...
std_dev_percchildbelowpovert_percbelowpoverty <- midwest |>
  group_by(state) |>
  summarize(
    sd_percchildbelowpovert = sd(percchildbelowpovert, na.rm = TRUE),
...
setosa_filter_avg_sepal_length <- mean(setosa_filter$Sepal.Length, na.rm = TRUE)
...
sep_width_avg_by_species <- iris |>
  group_by(Species) |>
  summarize(Average_Sepal_Width = mean(Sepal.Width, na.rm = TRUE))
...
avg_sepal_length_petal_length_by_species <- iris |>
  group_by(Species)|>
  summarize(Average_Sepal_Length = mean(Sepal.Length, na.rm = TRUE),
