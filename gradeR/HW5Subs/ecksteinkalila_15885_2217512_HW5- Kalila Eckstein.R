...
il_filter_percollege <- summarize(il_filter, mean(percollege))
...
std_dev_percchildbelowpovert_percbelowpoverty <- midwest |>
  group_by(state) |>
  summarize(sd(percchildbelowpovert), sd(percbelowpoverty))
...
setosa_filter_avg_sepal_length <- iris |>
  summarize((setosa_filter), mean(Sepal.Length))
...
avg_sepal_length_petal_length_by_species <- iris |>
  group_by(Species) |>
  summarize(mean(Sepal.Length), mean(Petal.Length))
...
more_than_100_hp_less_than_20_mpg_more_than_6_cyl <- more_than_100_hp |>
  filter(mpg > 20, cyl > 6) |>
  arrange(desc(mpg))
