...
il_filter <- filter(midwest, state == "IL")# Your code here
  ...
  il_filter_percollege <- mean(il_filter$percollege) # Your code here
  ...
  all_states_percollege <- midwest %>%
    group_by(state) %>%
    summarize(avg_percollege = mean(percollege))  # Your code here
  ...
  avg_percollege_percbelowpoverty <- midwest %>%
    group_by(state) %>%
    summarize(avg_percollege = mean(percollege), avg_percbelowpoverty = mean(percbelowpoverty)) # Your code here
  
  # calculate the median population (popadults) for each state. use the median() function
  median_popadults <- midwest %>%
    group_by(state) %>%
    summarize(median_popadults = median(popadults)) # Your code here
  ...
  std_dev_percchildbelowpovert_percbelowpoverty <- midwest %>%
    group_by(state) %>%
    summarize(sd_percchildbelowpoverty = sd(percchildbelowpovert), sd_percbelowpoverty = sd(percbelowpoverty)) # Your code here
...
setosa_filter <- filter(iris, Species == "setosa")# Your code here
  
  # Can you find the average of the Sepal.Length column for the filtered dataset (setosa_filter)?
  # Use the mean() function
  
  setosa_filter_avg_sepal_length <- mean(setosa_filter$Sepal.Length) # Your code here
  ...
  sep_width_avg_by_species <- iris %>%
    group_by(Species) %>%
    summarize(avg_sepal_width = mean(Sepal.Width))# Your code here
  ...
  avg_sepal_length_petal_length_by_species <- iris %>%
    group_by(Species) %>%
    summarize(avg_sepal_length = mean(Sepal.Length), avg_petal_length = mean(Petal.Length))# Your code here
  
  # Mtcars dataset
  ?mtcars # to give you an idea of the columns in the mtcars dataset

# Filter the cars that has more than 100 horsepower
more_than_100_hp <- filter(mtcars, hp > 100) # Your code here
  
  # Filter the cars that has more than 100 horsepower and has more than 4 cylinders
  more_than_100_hp_more_than_4_cyl <- filter(mtcars, hp > 100, cyl > 4)# Your code here
