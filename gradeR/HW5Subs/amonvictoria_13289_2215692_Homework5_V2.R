...
il_filter <- midwest|>
  filter(state == "IL")# Your code here
...
il_filter_percollege <- il_filter|>
  summarize(mean_percollegeIL = mean(percollege))# Your code here
...
all_states_percollege <- midwest|>
  summarize(mean_percollege = mean(percollege))# Your code here
...
avg_percollege_percbelowpoverty <- midwest|>
  summarize(avg_percollege = mean(percollege), 
            avg_poverty = mean(percbelowpoverty))# Your code here
...
std_dev_percchildbelowpovert_percbelowpoverty <- midwest|>
  group_by(state)|>
  summarize(sd_perchildbelowpovert = sd(percchildbelowpovert), 
            sd_percbelowpoverty = sd(percbelowpoverty))# Your code here
...
setosa_filter <-  iris|>
  filter(Species == "setosa")# Your code here
...
setosa_filter_avg_sepal_length <- setosa_filter|>
  summarise(avg_sepal_length = mean(Sepal.Length))# Your code here
...
sep_width_avg_by_species <- iris|>
  group_by(Species)|>
  summarise(avg_sep_width_species = mean(Sepal.Width))# Your code here
...
avg_sepal_length_petal_length_by_species <- iris|>
  ...
  summarise(avg_sepal_length = mean(Sepal.Length),
            avg_petal_length = mean(Petal.Length))# Your code here
...
avg_mpg_by_cyl_gear_carb <- mtcars|>
  ...
  summarize(avg_mpg_cyl_gear_carb = mean(mpg))|>
