...
std_dev_percchildbelowpovert_percbelowpoverty <- midwest |>
  group_by(state) |>
  summarise(sd_percchildbelowpovert = sd( percchildbelowpovert),
...
sep_width_avg_by_species <- iris |>
  group_by(Species) |>
  summarize(avg_sepal.Width = mean(Sepal.Width, na.rm = TRUE))
...
avg_sepal_length_petal_length_by_species <- iris |>
  group_by(Species) |>
  summarize( avg_SepalLength = mean(Sepal.Length, na.rm = TRUE),
  avg_Petal.Length= mean(Petal.Length, na.rm = TRUE))
...
avg_mpg_by_cyl_gear <- mtcars |>
  group_by(cyl,gear) |>
  summarise(avg_mpg_cylandgears = mean(mpg, na.rm = TRUE))
...
avg_mpg_by_cyl_gear_carb <- mtcars |>
  ...
  summarise( AVg_mpg_cly_gears_carb = mean( mpg,na.rm = TRUE)) |>
  arrange(cyl)
