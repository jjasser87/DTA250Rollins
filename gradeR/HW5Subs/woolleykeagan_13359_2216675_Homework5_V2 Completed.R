...
all_states_percollege <- midwest |>
  group_by(state) |> 
  summarise(all_states_percollege = mean(percollege, na.rm = TRUE))
...
std_dev_percchildbelowpovert_percbelowpoverty <- midwest |> 
  group_by(state)|>
  summarise(stdev_percchildbelowpovert = sd(percchildbelowpovert),
            stdev_percbelowpoverty = sd(percbelowpoverty))
  

# Iris dstate# Iris dataset
...
avg_mpg_by_cyl_gear_carb <- mtcars |>
  ...
  summarise(avg_mpg_cyl_gear_carb = mean(mpg)) |>
  arrange(cyl)
