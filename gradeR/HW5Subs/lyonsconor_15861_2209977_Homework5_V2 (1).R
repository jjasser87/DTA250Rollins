...
il_filter_percollege <- Mean(il_filter$percollege, na.rm = TRUE)
...
group_by(state) %>%
summarize(average_percollege = mean(percollege, na.rm = TRUE))
  
# calculate the median population (popadults) for each state. use the median() function
median_popadults <- median_popadults <- midwest %>%
group_by(state) %>%
summarize(median_popadults = median (popadults, na.rm = TRUE))
...
std_dev_percchildbelowpovert_percbelowpoverty <- std_dev_percchildbelowpovert_percbelowpoverty <- midwest %>% 
  group_by(state) %>% 
  summarize(sd_percchildbelowpovert = sd(percchildbelowpovert, na.rm = TRUE),
            sd_percbelowpoverty = sd(percbelowpoverty, na.rm = TRUE)
# Iris dataset
...
setosa_filter_avg_sepal_length <- setosa_filter_avg_sepal_length <- mean(setosa_filter$Sepa1.Length, na.rm = TRUE)
...
group_by(Species) %>%
summarize(average_sepal_width = mean (Sepal.Width, na.tm = TRUE))
...
avg_sepal_length_petal_length_by_species <- group_by(Species) %>%
summarize(average_sepal_length) = meanSepal.Length, na.rm = TRUE),
average_petal_length = mean(Petal.Length, na.rm = TRUE))
...
avg_mpg_by_cyl <- avg_mpg_by_cyl <- mtcars %>%
group_by(cyl) %>%
...
avg_mpg_by_cyl_gear <- avg_mpg_by_cyl_gear <- mtcars
group_by(cyl, gear) %>%
summarize(average_mpg = mean(mpg, na.rm = TRUE)) %>%
