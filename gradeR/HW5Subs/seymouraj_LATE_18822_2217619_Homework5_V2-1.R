...
il_filter_percollege <- il_filter |>
  summarise(mean_percollege = mean(percollege))
...
avg_percollege_percbelowpoverty <- df |>
  group_by(percollege, percbelowpoverty) |>
  summarise(mean_percbelowpoverty = mean(percbelowpoverty), mean_percollege = mean(percollege))
  
# calculate the median population (popadults) for each state. use the median() function

median_popadults <- function(data, var) {
  data |> summarise(
    median= median({{var}}, na.rm = T)
  )
}

midwest |> median_popadults(popadults)
...
std_dev_percchildbelowpovert_percbelowpoverty <- function(data, var) {
  data |> summarise(
    sd= sd({{var}}, na.rm = T)
  )
}

midwest |> std_dev_percchildbelowpovert_percbelowpoverty(percchildbelowpovert)
midwest |> std_dev_percchildbelowpovert_percbelowpoverty(percbelowpoverty)
...
avg_sepal_length_petal_length_by_species <- df |>
  group_by(Sepal.Length, Petal.Length) |>
  summarise(mean_Sepal.Length = mean(Sepal.Length), mean_Petal.Length = mean(Petal.Length))
...
more_than_100_hp_more_than_4_cyl <- df |> 
  filter(hp > 100 & cyl > 4)
...
more_than_100_hp_less_than_20_mpg_more_than_6_cyl <- df |>
